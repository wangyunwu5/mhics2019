...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
		...
		//查询用户是否存在
		Person user = userService.getOneByAccount(account);
		if(user == null) {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		for (Role role : roles) {
			if(role != null) {
				...
				List<SystemMenu> resources = userService.getSystemMenuByRole(role.getRoleId());
				for (SystemMenu resource : resources) {
					if(resource != null) {
						simpleAuthorizationInfo.addStringPermission((resource.getMenuId()).toString());
					}
				}
			}
		}
