...
<body style="background-color: #ecf0f5;">
	...
	<div class="row" style="padding-top: 15px;">
		<div class="col-md-12">
			<!--id="container"-->
			<div class="bgc-w box">
				<form action="projecteditsave" method="post" onsubmit=";">
					<#if project??>
							...
							<div class="row">
								<#if project??>
									<div class="col-md-6 form-group">
										...
										<input name="projectNo" readonly="readonly" class="form-control" value="${(project.projectNo)!''}"/>
									</div>
								<#else>
									<div class="col-md-6 form-group">
										<label class="control-label"><span>项目编号</span></label> 
										<input name="projectNo" class="form-control" value="${(project.projectNo)!''}"/>
									</div>
								</#if>
								<div class="col-md-6 form-group">
									<label class="control-label"><span>地图区域代码</span></label> 
									<input name="drainageAreaCode" class="form-control" value="${(project.drainageAreaCode)!''}"/>
								</div>
								<div class="col-md-6 form-group">
									<label class="control-label"> <span>创建人</span></label>
									 <select  class="form-control" name="persononeid">
										<#if project??>
											<option value="${(project.personOne.userId)!''}">${project.personOne.userAccount}</option>
										<#else>	
											<#list users as user>
								...
								<#if project??>
									<div class="col-md-6 form-group">
										<span> <label class="control-label">创建时间</label>
										</span> <input name="createTime" readonly="readonly" class="form-control" id="start" onclick=""
											value="${(project.createTime)!''}"/>
									</div>
								<#else>
									<div class="col-md-6 form-group">
										<span> <label class="control-label">创建时间</label>
										</span> <input name="createTime" class="form-control" id="start" onclick="WdatePicker()"
											value="${(project.createTime)!''}"/>
									</div>
								</#if>
								<div class="col-md-6 form-group">
									<label class="control-label"> <span>项目负责人</span></label>
									 <select class="form-control" name="persontwoid">
										<#if project??>
											<option value="${(project.personTwo.userId)!''}">${project.personTwo.userAccount}</option>
										</#if>
										<#list users as user>
											<option value="${user.userId}">${user.userAccount}</option>
										</#list>
